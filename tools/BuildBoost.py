...
def main(argv):
    ...
    build64 = '--no64' not in argv
    build86 = '--no86' not in argv

    buildDebug = '--noDebug' not in argv
    buildRelease = '--noRelease' not in argv
...
def build(boostPath, pythonPath, architecture, buildDebug, buildRelease):
    ...
    with open(configPath, "a") as config:  # For now version is hardcoded!
        config.write(configString.format('3.6', pythonPath.replace('\\', '\\\\'),
                                         os.path.join(pythonPath, 'include').replace('\\', '\\\\'),
                                         os.path.join(pythonPath, 'libs').replace('\\', '\\\\')))
